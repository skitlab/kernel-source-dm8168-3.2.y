...
static void ti81xx_msi_handler(unsigned int irq, struct irq_desc *desc)
{
	 ...
	 */
	if (likely(desc->irq_data.chip->irq_ack))
...
static void ack_msi(struct irq_data *data)
{
	int irq = data->irq;
 ...
 */
static void ack_msi_inv(struct irq_data *data)
{
	int irq = data->irq;
	unsigned int msi_num = irq - msi_irq_base;
	writel(~(1 << (msi_num & 0x1f)), reg_virt + MSI0_IRQ_STATUS);
...
static void mask_msi(struct irq_data *data)
{
	int irq = data->irq;
	unsigned int msi_num = irq - msi_irq_base;
...
static void unmask_msi(struct irq_data *data)
{
	int irq = data->irq;
	unsigned int msi_num = irq - msi_irq_base;
...
int arch_setup_msi_irq(struct pci_dev *pdev, struct msi_desc *desc)
{
	...
	if (ret < 0) {
		...
		if (!ret) {
			...
			irq_set_chip(irq, &ti81xx_msi_chip);
...
static int ti81xx_pcie_setup(int nr, struct pci_sys_data *sys)
{
	...
	if ((msi_irq >= 0) && msi_irq_num) {
		...
		if (msi_inv)
			ti81xx_msi_chip.irq_ack = ack_msi_inv;

		irq_set_chained_handler(msi_irq, ti81xx_msi_handler);
	} else {
 ...
 * single IRQ.
 */
static int ti81xx_pcie_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
