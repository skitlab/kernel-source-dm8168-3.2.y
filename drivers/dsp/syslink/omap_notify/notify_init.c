...
notify_add_mmu_entry(void *mmu_handle, u32 slave_virt_addr, u32 size)
{
	...
	do {
		...
		omap_iopgtable_lookup_entry(mmu_handle,
					slave_virt_addr,
					(u32 **) &ppgd,
					(u32 **) &ppte);
		if (!*ppgd || !*ppte) {
			...
			if (omap_iopgtable_store_entry(mmu_handle, &tlb_entry)) {
			...
			if (master_phys_addr != (*ppte & IOPTE_MASK)) {
				...
				tlb_entry.da = slave_virt_addr;
				tlb_entry.pa = master_phys_addr;
				if (omap_iopgtable_store_entry(mmu_handle,
								&tlb_entry)) {
						printk(KERN_ERR "ERROR in"
						"notify_add_mmu_entry\n");
					status = -1;
...
static int __init notify_init(void)
{
	...
	if (dsp_notify_va != 0) {
		...
		if (i != MULTIPROC_INVALIDID) {
			...
			if (cpu_is_omap343x() && !cpu_is_ti81xx()) {
...
static void __exit notify_exit(void)
{
	...
	while (notifies[i] != NULL) {
		...
		if (notifies[i]->mmu_handle) {
			iommu_detach_device(notifies[i]->mmu_handle->domain,
								notifies[i]->mmu_handle->dev);
			iommu_domain_free(notifies[i]->mmu_handle->domain);
		}
	}
}
